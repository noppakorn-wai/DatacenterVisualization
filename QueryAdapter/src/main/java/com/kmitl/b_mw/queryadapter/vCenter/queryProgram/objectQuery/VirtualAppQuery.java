...
/**
 ...
 @author B_MW (Wai & May)
 */
public class VirtualAppQuery implements Queryable
{

    private static final Logger LOG = Logger.getLogger(VirtualAppQuery.class.getName());
    private final Session session;

    public VirtualAppQuery(Session session)
    {
    ...
    {
        LOG.info("Start VirtualAppQuery.run");
        ...
        includeFields.add("newwork.MOR.val");
        ...
        includeFields.add("VAppConfig.*");
        includeFields.add("values");
        includeFields.add("VMs.MOR.val");

        JSONObject json = session.query(VirtualApp.class, includeFields.toArray(new String[0]));
